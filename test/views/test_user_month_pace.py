...
MONTHLY_200 = 200 * 100.0  # $200 plan = 20000 credits / month
...
def test_pace_projects_mtd_to_full_month(fake_source, make_record):
    ...
    assert pace["pct"] == approx(pace["projected"] / MONTHLY_200, rel=1e-3)
    assert pace["delta"] == approx(pace["projected"] - MONTHLY_200, abs=0.05)
...
def test_no_pace_before_five_days_of_data(fake_source, make_record):
    ...
    v = ac.user_view(fake_source(rows), "alice", "$200 / month", "2026-07")
...
def test_pace_status_over_when_projection_exceeds_allowance(fake_source, make_record):
    ...
    v = ac.user_view(fake_source(rows), "alice", "$200 / month", "2026-07")
    pace = v["month_pace"]
    assert pace["projected"] == approx(6000.0 / 6 * 31, abs=0.5)  # 31000 >> 20000
    assert pace["status"] == "over"
    assert pace["delta"] > 0


def test_pace_status_on_track_near_allowance(fake_source, make_record):
    # 3870.9 credits over 6 of 31 days projects to ~20000 (within ±2%).
    rows = [make_record("2026-07-06", "alice", 3870.9)]
    v = ac.user_view(fake_source(rows), "alice", "$200 / month", "2026-07")
...
def test_no_pace_when_latest_day_is_month_end(fake_source, make_record):
    ...
    v = ac.user_view(fake_source(rows), "alice", "$200 / month", "2026-06")
    assert v["month_pace"] is None
